...
def add_func(expression, combinations, design_miRNA_dict):
    """This functions takes expression values caused by a single microRNA, microRNA combinations, and a dictionary
    that maps microRNA combinations to design names. It returns the expression values caused by the microRNA combinations.
    
    expression: dataframe with expression values caused by a single microRNA (index: microRNA names, columns: cell lines)
    ...
    add = additive_expression(expression, combinations)
